...
import {
  deleteVisitorVans,
  newOrUpdatedVisitorVans,
  visitorVansPageList,
  visitorVansPageListType
} from "@/api";
...
const dataList = ref<visitorVansPageListType[]>([]);
let backups: visitorVansPageListType[] = [];
...
const saveFn = (data: any) => {
  Promise.all(refList.value.map(v => v.validate()))
    .then(() => {
      return newOrUpdatedVisitorVans(data.row);
    })
    .then(() => {
      editIndex.value = -1;
      getData();
