...
<template>
  <div class="bg-white dark:bg-[#141414] content">
    <div class="c_box">
      <div class="title">
        <div class="lf">{{ t('system.setRole.title') }}</div>
      </div>
      <div class="form_box_p">
        <div class="form_box">
          <div class="form_item_box">
            <div>{{ t('system.setRole.fields.account') }}：</div>
            <el-input
              v-model="searchParams.account"
              clearable
              :placeholder="t('system.setRole.placeholders.account')"
            />
          </div>
          <div class="form_item_box">
            <div>{{ t('system.setRole.fields.userName') }}：</div>
            <el-input
              v-model="searchParams.userName"
              clearable
              :placeholder="t('system.setRole.placeholders.userName')"
            />
          </div>
        </div>
      ...
      <el-table :data="dataList" :height="tableHeight" :style="{ width: '100%' }">
        ...
        <el-table-column align="center" :label="t('system.setRole.columns.account')">
          <template #default="scope">
            <div class="text_box">{{ scope.row.account ?? "--" }}</div>
          </template>
        </el-table-column>
        <el-table-column align="center" :label="t('system.setRole.columns.userName')">
          <template #default="scope">
            <div class="text_box">{{ scope.row.userName ?? "--" }}</div>
          </template>
        </el-table-column>
        <el-table-column align="center" :label="t('system.setRole.columns.role')">
          <template #default="scope">
            <div class="text_box">
              {{ scope.row.roleName ?? "--" }}
            </div>
        ...
        <el-table-column align="center" :label="t('system.setRole.columns.phoneNumber')">
          <template #default="scope">
            <div class="text_box">
        ...
        <el-table-column align="center" :label="t('system.setRole.columns.freeTime')">
          <template #default="scope">
            <div class="text_box">
              {{ scope.row.freeTime ?? "--" }}
            </div>
        ...
        <el-table-column align="center" :label="t('label.operation')" width="265">
          <template #default="scope">
            <el-form-item prop="url">
              ...
              >
                {{ t('system.setRole.actions.resetPassword') }}
              </el-button>
            </el-form-item>
          </template>
        </el-table-column>
